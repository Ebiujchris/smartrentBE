...
export class AdminService {
  ...
  async getUserGrowthData() {
    ...
    // Group by month
    const monthlyData: Record<string, { landlords: number; tenants: number; total: number }> = {};
    users.forEach((user) => {
  ...
  async replyToMessage(messageId: string, adminId: string, dto: ReplyMessageDto) {
  ...
  // ─── Properties ───────────────────────────────────────────────────
  async getProperties(query: { page?: number; limit?: number; search?: string }) {
    const page = query.page || 1;
    const limit = query.limit || 20;
    const skip = (page - 1) * limit;
  ...
  async getActivityLog(limit = 30) {
    // Combine recent registrations, payments, and maintenance requests
